...
def train_on_device(obj_names, args):
    ...
    for obj_name in obj_names:
        ...
        run_name = run_name_top + 'DRAEM_' + obj_name
        ...
        dataset = MVTecDRAEMTrainDataset(args.data_path + obj_name + "/train/", None,
                                         resize_shape=[256, 256]) 

        dataloader = DataLoader(dataset, batch_size=args.batch_size,
                                shuffle=False, num_workers=args.num_workers) # ,worker_init_fn=worker_init_fn
        ...
        for epoch in range(args.epochs):
            ...
            for i_batch, sample_batched in enumerate(dataloader):
                ...
                if (epoch * len(dataloader) + i_batch) % 50 == 49:  # 每 1000 的数据批量
                    # 记录运行的损失值（loss）
            ...
            if args.checkpoint_interval != -1 and (epoch + 1) % args.checkpoint_interval == 0 and (epoch + 1) >= 490: # and (epoch + 1) >= 490
                save_name = run_name + "_epoch" + str(epoch + 1) + "_solu3"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--obj_id', action='store', type=int, default=0) # selected obj_id
    ...
    parser.add_argument('--data_path', action='store', type=str, default=r'/home/dataset/Visa_pytorch_foreground/1cls/')   # your VisA w/ its foreground path!
